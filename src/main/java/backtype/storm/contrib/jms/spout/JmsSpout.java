 ...
 * implementations to obtain the JMS <code>ConnectionFactory</code> and
 * <code>Destination</code> objects necessary to connect to a JMS topic/queue.
 * <p/>
...
public class JmsSpout extends BaseRichSpout implements MessageListener {
	...
	/**
	 * Sets the JMS Session acknowledgement mode for the JMS seesion associated
	 * with this spout.
	 * <p/>
	 * Possible values:
	 * <ul>
	...
	public void setJmsAcknowledgeMode(int mode) {
		...
		case Session.DUPS_OK_ACKNOWLEDGE:
			break;
		default:
			throw new IllegalArgumentException("Unknown Acknowledge mode: "
	 ...
	 * implementation that will convert <code>javax.jms.Message</code> object to
	 * <code>backtype.storm.tuple.Values</code> objects to be emitted.
	 * 
	 * @param producer
	 */
	 ...
	 * <p/>
	 * Stored the JMS message in an internal queue for processing by the
	...
	public void open(Map conf, TopologyContext context,
		...
		Integer topologyTimeout = (Integer) conf
				.get("topology.message.timeout.secs");
		// TODO fine a way to get the default timeout from storm, so we're not
		// hard-coding to 30 seconds (it could change)
		topologyTimeout = topologyTimeout == null ? 30 : topologyTimeout;
		if ((topologyTimeout.intValue() * 1000) > this.recoveryPeriod) {
			LOG.warn("*** WARNING *** : " + "Recovery period ("
					+ this.recoveryPeriod
		...
		try {
			...
			if (this.isDurableSubscription() && this.recoveryPeriod > 0) {
				...
				this.recoveryTimer.scheduleAtFixedRate(new RecoveryTask(), 10,
						this.recoveryPeriod);
			}
	 ...
	 * <p/>
	 * Setting this value to <code>false</code> essentially means this spout
	 * will run as a singleton within the cluster ("parallelism_hint" will be
	 * ignored).
	 * <p/>
	 * In general, this should be set to <code>false</code> if the underlying
